<script lang="ts">
	...
	// label the local minima: the hard days
	...
	const peak = scored.reduce((m, d) => (d.sentiment! > m.sentiment! ? d : m), scored[0]);
...
<figure bind:clientWidth={width}>
	<svg viewBox="0 0 {width} {H}" role="img" aria-label="Sentiment of the dev notes across the course days">
		...
		{#if width >= 480}
			{#each dips as d}
				<text x={clampX(x(scored.indexOf(d)))} y={y(d.sentiment!) + 26} class="note">
					“{shortTitle(d.title)}”
				</text>
			{/each}
		{/if}
		<text x={clampX(x(scored.indexOf(peak)))} y={y(peak.sentiment!) - 14} class="note peak">
			“{shortTitle(peak.title)}”
		</text>
